...
(function (factory) {
  ...
  var Dialog = function () {
    ...
    this.showImageDialog = function ($editable, $dialog) {
      return $.Deferred(function (deferred) {
        var $imageDialog = $('#note-image-dialog');

        var $imageInput = $dialog.find('.note-image-input'),
            $imageUrl = $dialog.find('.note-image-url'),
            $imageBtn = $dialog.find('.note-image-btn');

        $(document).one('opened.fndtn.reveal', '#note-image-dialog', function () {
          // Cloning imageInput to clear element.
          $imageInput.replaceWith($imageInput.clone()
            .on('change', function () {
              deferred.resolve(this.files || this.value);
              $imageDialog.foundation('reveal', 'close');
            })
            .val('')
          );
        ...
        $(document).one('closed.fndtn.reveal', '#note-image-dialog', function () {
    ...
    this.showLinkDialog = function ($editable, $dialog, linkInfo) {
      return $.Deferred(function (deferred) {
        ...
        $(document).one('opened.fndtn.reveal', '#note-link-dialog', function () {
        ...
        $(document).one('closed.fndtn.reveal', function () {
          // detach events
          $linkText.off('input');
          $linkUrl.off('input');
          $linkBtn.off('click');
    ...
    this.showHelpDialog = function () {
      return $.Deferred(function (deferred) {
        var $helpDialog = $('#note-help-dialog');

        $(document).one('closed.fndtn.reveal', '#note-help-dialog', function () {
          deferred.resolve();
        });
  ...
  var Renderer = function () {
    ...
    var tplButton = function (label, options) {
      ...
      var dropdownId = (dropdown) ? dropdown.replace(/^<ul id=\"(\w+)\".+/, '$1') : '';
      var dialog = options.dialog;
      var a = options.a;

      var hide = options.hide;

      return (a ? '<a href="#"' : '<button type="button"') +
                 ' class="button tiny secondary' +
                   (className ? ' ' + className : '') + '"' +
                   (dropdown ? ' data-dropdown="' + dropdownId + '" aria-controls="' + dropdownId + '" aria-expanded="false"' : '') +
                 (title ? ' title="' + title + '"' : '') +
                 (dialog ? ' data-reveal-id="' + dialog + '"' : '') +
    ...
    var tplPopover = function (className, content) {
      return '<div class="' + className + ' f-dropdown popover" style="display: none;">' +
               '<div class="arrow"></div>' +
               '<div class="popover-content button-bar">' +
                 content +
               '</div>' +
             '</div>';
    };
    ...
    var tplDialog = function (className, title, body, footer) {
      return '<div id="' + className + '" class="reveal-modal" aria-hidden="false" data-reveal>' +
              '<h4>' + title + '</h4>' +
    ...
    var tplButtonInfo = {
      ...
      table: function (lang) {
        var dropdown = '<ul id="dropdownTable" class="note-table f-dropdown" data-dropdown-content aria-hidden="true" tabindex="-1">' +
      ...
      style: function (lang, options) {
        ...
        return tplIconButton('fa fa-magic', {
          ...
          dropdown: '<ul id="dropdownStyles" class="f-dropdown dropdown-menu"' +
            ' data-dropdown-content aria-hidden="true" tabindex="-1">' + items + '</ul>'
      ...
      fontname: function (lang, options) {
        ...
        return tplButton(label, {
          ...
          dropdown: '<ul id="dropdownFontname" class="f-dropdown dropdown-menu"' +
            'data-dropdown-content aria-hidden="true" tabindex="-1">' + items + '</ul>'
        });
      },
      color: function (lang) {
        ...
        var dropdown = '<ul id="dropdownColor" class="f-dropdown dropdown-menu ' +
          'large-block-grid-2" data-dropdown-content aria-hidden="true" tabindex="-1">' +
                         '<li>' +
      ...
      paragraph: function (lang) {
        ...
        var dropdown = '<ul id="dropdownParagraph" class="f-dropdown ' +
          'dropdown-menu" data-dropdown-content aria-hidden="true" tabindex="-1">' +
                         '<li class="note-align radius button-group">' +
                           leftButton + centerButton + rightButton + justifyButton +
                         '</li>' +
                         '<li class="note-list radius button-group">' +
                           indentButton + outdentButton +
                         '</li>' +
                       '</ul>';

        return tplIconButton('fa fa-paragraph', {
      ...
      height: function (lang, options) {
        ...
        return tplIconButton('fa fa-text-height', {
          ...
          dropdown: '<ul id="dropdownHeight" class="f-dropdown ' +
            'dropdown-menu" data-dropdown-content aria-hidden="true" tabindex="-1">' + items + '</ul>'
        });
    ...
    var tplPopovers = function (lang, options) {
      var tplLinkPopover = function () {
        ...
        var content = '<a href="http://www.google.com" target="_blank">www.google.com</a>&nbsp;&nbsp;' +
                      '<div class="note-insert button-group radius">' +
      ...
      var tplImagePopover = function () {
        ...
        var content = '<div class="button-group radius">' + fullButton + halfButton + quarterButton + '</div>' +
                      '<div class="button-group radius">' + leftButton + rightButton + justifyButton + '</div>' +
                      '<div class="button-group radius">' + roundedButton + circleButton + thumbnailButton + noneButton + '</div>' +
                      '<div class="button-group radius">' + removeButton + '</div>';
      ...
      var tplAirPopover = function () {
        ...
        for (var idx = 0, len = options.airPopover.length; idx < len; idx ++) {
          ...
          content += '<div class="note-' + group[0] + ' button-group radius">';
    ...
    var tplShortcut = function (title, keys) {
      var keyClass = 'columns large-6 note-shortcut-col note-shortcut-';
      var body = [];
      ...
      return '<div class="note-shortcut-row row"><div class="' + keyClass + 'title large-offset-6">' + title + '</div></div>' +
    ...
    var tplShortcutTable = function (lang, options) {
      var colClass = 'class="note-shortcut note-shortcut-col columns large-6"';
      var template = [
    ...
    var tplDialogInfo = {
      image: function (lang, options) {
        ...
        var body = '<div class="form-group row-fluid note-group-select-from-files">' +
                   ...
                   '</div>';
        var footer = '<button class="note-image-btn disabled" disabled>' + lang.image.insert + '</button>';
      ...
      link: function (lang, options) {
        ...
        var footer = '<button href="#" class="note-link-btn disabled" disabled>' + lang.link.insert + '</button>';
        return tplDialog('note-link-dialog', lang.link.insert, body, footer);
      },

      help: function (lang, options) {
        var body = '<div class="title">' + lang.shortcut.shortcuts + '</div>' +
     ...
     */
    var createTooltip = function ($container, keyMap) {
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      for (var idx = 0, len = options.toolbar.length; idx < len; idx ++) {
        ...
        toolbarHTML += '<div class="note-' + groupName + ' button-group radius">';
      ...
      toolbarHTML = '<div class="note-toolbar button-bar">' + toolbarHTML + '</div>';
